...
export const instructor = sqliteTable('instructor', {
  ...
  availableForMentoring: integer('available_for_mentoring', { mode: 'boolean' }),
  hourlyRate: integer('hourly_rate', { mode: 'number' })
});
...
export const courseProgress = sqliteTable('course_progress', {
  ...
  userId: integer('user_id').notNull().references(() => user.id),
  courseId: integer('course_id').notNull().references(() => course.id),
  completionPercentage: integer('completion_percentage', { mode: 'number' }).notNull(),
